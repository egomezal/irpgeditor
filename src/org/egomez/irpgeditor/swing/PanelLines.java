...
public class PanelLines extends JPanel implements Runnable, DocumentListener {
  ...
  public void parse() {
    ...
    while ( line != null ) {
      if ( line.isComment() == false && line.isInvalid() == false &&
           line.isDirective() == false && line.isSql() == false ) {
        spec = line.getSpec();
        if ( spec == 'C' || spec == 'c' ) {
...
try {
  ...
  class Block {
    int startType, endType; // 0 = IF, 1 = SELECT, 2 = BEGSR, 3 = DO, 4 = MONITOR, 5 = FOR, 6 = END
