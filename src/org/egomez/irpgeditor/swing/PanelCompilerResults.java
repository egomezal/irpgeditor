...
//      putValue(Action.MNEMONIC_KEY, new Character('S'));
  ...
  /**
   * listens for mouse double clicks on the compiler results.
   * determines if the user clicked on a line that has text that describes a
   * compiler error, if so then goes to that line in the source and highlights the error.
   */
  class MouseAdapterCompilerResults extends MouseAdapter {
    public void mouseClicked(MouseEvent evt) {
      ...
      if ( evt.getClickCount() > 1 ) {
        ...
        // get the line start, position start, line end, and position end of the code
        // that is in error.
        ...
        // the next 5 tokens are placed by the system, not sure what they are.
        for ( int x = 0; x < 4; x++ ) {
          tokenizer.nextToken();
          if ( !tokenizer.hasMoreTokens() ) {
            return;
