              ...
              for ( int f = 0; f < listFiles.size(); f++ ) {
                ...
                try {
                  ...
                  for ( int m = 0; m < listMembers.size(); m++ ) {
                    ...
                    setText(label4, "Member: " + member.getName() + " (" + (m + 1) + " of " + listMembers.size() + ")");
                    try {
                      listResults = system.search(lib, file, member.getName(), textfieldScanTerm.getText(), false);
                      if ( stop ) return;
                      for ( int x = 0; x < listResults.size(); x++ ) {
                        append("FOUND: " + system.getName() + ", " + lib + ", " + file + ", " + member.getName() + ", " + listResults.get(x) + "\n");
                      }
                    }
                    catch (Exception e) {
                      e.printStackTrace();
//                      JOptionPane.showMessageDialog(FrameRPGEditor.frameRPGEditor, e.getMessage());
                    }
                  }
                }
                catch (Exception e) {
