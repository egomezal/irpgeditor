...
public class PanelLayout extends PanelTool implements Runnable, ClosableTab {
  ...
  public void run() {
    ...
    try {
      ...
      // dont want this program modification of the combobox of libraries to cause a lookup of data.
  ...
  class ActionOpenSource implements ActionListener {
    public void actionPerformed(ActionEvent evt) {
      ...
      member = new Member(Environment.systems.getDefault(), tableModelInfo.getSourceLibrary(), tableModelInfo.getSourceFile(), tableModelInfo.getSourceMember());
  ...
  class ActionCreateLogical implements ActionListener {
    public void actionPerformed(ActionEvent evt) {
      ...
      member = DialogMemberNew.showDialog(null, as400, null, null, null, "LF");
      if ( member == null ) {
        return;
      }
      panelMember = (PanelMember)Environment.members.open(member);
      if ( panelMember == null ) {
        return;
      }
      buffer = new StringBuffer("     A          R                           PFILE(" + tableName + ")\n");
      SourceLine.formatText(buffer, LinePosition.A_NAME, (String)tableFormats.getValueAt(0, 0));
      rows = table.getSelectedRows();
      for ( int x = 0; x < rows.length; x++ ) {
        row = rows[x];
        buffer.append("     A          K ");
        buffer.append(tableModel.getValueAt(row, 0));
        buffer.append("\n");
      }
  ...
  class ActionSqlGenerate implements ActionListener {
    public void actionPerformed(ActionEvent evt) {
      ...
      if ( text.indexOf("?") == -1 ) {
        Environment.sql.executeSQL(text.toString() + "\n");
      }
      else {
        Environment.sql.appendSQL(text.toString() + "\n");
      }
