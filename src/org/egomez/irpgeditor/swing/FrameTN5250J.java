...
public class FrameTN5250J extends JFrame {
  ...
  public void run(RunConfiguration config) {
    ...
    try {
      connect();
      sendKeys(as400system.getUser() + TN5250jConstants.MNEMONIC_FIELD_EXIT + as400system.getPassword() + TN5250jConstants.MNEMONIC_ENTER, true);
      ...
      if ( config.libraries.trim().length() > 0 ) {
        ...
        while ( tokenizer.hasMoreTokens() ) {
          ...
          if ( token.startsWith("*") ) {
            ...
            text = "ADDLIBLE " + token + " POSITION(" + position + ")" + TN5250jConstants.MNEMONIC_FIELD_PLUS + TN5250jConstants.MNEMONIC_ENTER;
            sendKeys(text, true);
          }
        }
      }
      ...
      if ( member != null && config.debug.trim().length() > 0 ) {
        ...
        if ( member.getCompileType().equalsIgnoreCase("CRTRPGPGM") ) {
          ...
          // return because a call isnt needed with this version of debug.
