...
public class PanelModuleBrowser extends JPanel {
  ...
  class NodeLibrary extends NodeDefault {
    ...
    protected void retrieveBindingDirectories() {
      ...
      isRetrieving = true;
      try {
        dirs = as400.listObjects(text, "*BNDDIR");
        for ( int x = 0; x < dirs.size(); x++ ) {
          add(new NodeBindingDirectory(this, (BindingDirectory)dirs.get(x), treeModel));
        }
        list.remove(nodeWait);
      }
      catch (Exception e) {
  ...
  class NodeBindingDirectoryEntry extends NodeDefault {
    ...
    public String getToolTipText() {
      return bde.getBindingDirectory().getAS400().getName() + " - " + bde.getBindingDirectory().getLibrary() + " - " + bde.getBindingDirectory().getName() + " - " + text;
    }
    ...
    protected void retrieve() {
      ...
      try {
        if ( bde.getType().trim().equalsIgnoreCase("*SRVPGM") ) {
          temp = bde.getBindingDirectory().getAS400().listServiceProgramModules(bde.getLibrary(), bde.getName());
  ...
  class NodeModule extends NodeDefault {
    ...
    public void click(int count) {
      ...
      if ( module.getSourceLibrary().trim().length() == 0 ||
           module.getSourceFile().trim().length() == 0 ||
           module.getSourceMember().trim().length() == 0 ) {
        JOptionPane.showMessageDialog(null, "Unable to determine location of source code.");
        return;
      }
      Environment.members.open(new Member(module.getSystem(), module.getSourceLibrary(), module.getSourceFile(), module.getSourceMember()));
    }
