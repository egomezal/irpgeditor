...
public class PanelProjectMembers extends PanelTool {
  ...
  /**
   * listens for when the user selects a member from the tree in order
   * to enable or disable buttons and menus.
   */
  ...
  class ActionMemberDelete implements ActionListener {
    public void actionPerformed(ActionEvent evt) {
      ...
      if ( JOptionPane.showConfirmDialog(null, "Are You Sure?", "Delete Member(s)", JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.CANCEL_OPTION ) {
        return;
      }
  ...
  class ActionMemberOpen extends AbstractAction {
    public ActionMemberOpen() {
      super("Member Open", Icons.iconMemberOpen);
      setEnabled(false);
      putValue("MENU", "File");
      // F5 + CTRL
...
//      putValue(Action.MNEMONIC_KEY, new Character('S'));
  ...
  class ActionMemberRemove implements ActionListener {
    public void actionPerformed(ActionEvent evt) {
      ...
      if ( JOptionPane.showConfirmDialog(null, "Are You Sure?", "Remove Project Member(s)", JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.CANCEL_OPTION ) {
        return;
      }
      project = (Project)Environment.projects.getSelected();
      if ( project == null ) {
