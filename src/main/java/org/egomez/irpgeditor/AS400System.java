...
public class AS400System extends NodeAbstract {
    ...
    ArrayList<ListenerAS400System> listListeners = new ArrayList<>();
    ...
    ArrayList<String> listCallBuffer = new ArrayList<>();
     ...
     *
     * @param name String The name you use refer to this as400 system.
     * @param address String The i.p. address or url to use to connect.
     * @param user String The user name to use to connect.
     * @param password String The password to use to connect.
     */
    ...
    public ArrayList<BindingDirectory> listBindingDirectories(String library, String type) throws Exception {
        ...
        ArrayList<BindingDirectory> list = new ArrayList<>();
    ...
    public AS400 getAS400() {
        if (!as400.isConnectionAlive()) {
            try {
                as400.authenticate(this.user, this.password.toCharArray());
                connect();
     ...
     * jtopen project.
     *
     * @throws Exception if there is an error connecting to the as400.
     */
    public void connect() throws Exception {
        if (user != null && password != null) {
            disconnect();
            
            as400 = new AS400(address, user, password.toCharArray());
     ...
     *
     * @param listener ListenerAS400System Will be notified of events.
     */
     ...
     *
     * @param listener ListenerAS400System Will not be notified of events.
     */
     ...
     * members.
     *
     * @param library String The name of the library on the as400 to retrieve
     * the members from.
     * @param file String The name of the file on the as400 to retrieve the
     * members from.
     * @throws Exception If there is an error getting the members.
     ...
     *
     * @param library String The library to look for source files in.
     * @throws SQLException
     * @return ArrayList contains String objects;
     */
    ...
    public String getSourceMemberType(String library, String file, String member) throws Exception {
        ...
        ProgramCallDocument pcml1 = new ProgramCallDocument(this.as400, "api");
        pcml1.setValue("qusrmbrd.receiverLength", pcml1.getOutputsize("qusrmbrd.receiver"));
        pcml1.setValue("qusrmbrd.fileName", "" + buffer);
        pcml1.setValue("qusrmbrd.memberName", member);
        boolean result = pcml1.callProgram("qusrmbrd");
        if (!result) {
            Environment.qcmdexec.append(pcml1.getMessageList("qusrmbrd"), Color.RED, result);
            return null;
        }
        
        return pcml1.getValue("qusrmbrd.receiver.sourceType").toString();
    }
     ...
     * view sql command?)
     *
     * @param library String The library to look source files in.
     * @throws SQLException
     * @return ArrayList contains String objects;
     */
     ...
     *
     * @param library String The library to search in.
     * @param file String The file to search in.
     * @param member String The source member to search in.
     * @param term String The term/text to search for.
     * @param matchCase boolean If true then the case of the term specified must
     * match the case of the term in the source member.
     * @throws SQLException
     * @return ArrayList contains String objects.
     */
     ...
     *
     * @param library String The library name of the source member.
     * @param file String The file name of the source member.
     * @param member String The name of the source member.
     * @param sourceLoader SourceLoader Receives an event for each line that is
        ...
        synchronized (connection) {
            try (Statement stmt = connection.createStatement(); ResultSet rs = stmt
                    .executeQuery("select max(LENGTH) from syscolumns where table_name = '" + file.toUpperCase()
                            + "' and table_schema = '" + library.toUpperCase()
     ...
     * window.
     *
     * @param library String The library that contains the error text file.
     * @param file String The file that contains the error source member.
     * @param member String The name of the source member that contains the
     * errors.
     * @param path String The library/file/member that was compiled to produce
     * the errors.
     * @throws SQLException
    ...
    public int[] getIndex(String library, String file, String member) throws SQLException {
        ...
        synchronized (connection) {
            ...
            rs = stmt.executeQuery("select * from qtemp/a" + a);
            while (rs.next()) {
                list.add((int) Float.parseFloat(rs.getString(1)));
            }
            rs.close();
            stmt.execute("drop alias qtemp/a" + a);
            stmt.close();
            ...
            public void run() {
                try {
                    ...
                    try {
                        as400.setGuiAvailable(false);
                        if (!as400.isConnectionAlive()) {
                            as400.authenticate(getUser(), getPassword().toCharArray());
                        }
