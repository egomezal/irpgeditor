...
public class TableModelLayout extends AbstractTableModel {
  String schema, name;
  AS400System as400;
  String[] columnNames = new String[] { "NAME", "SIZE", "DESC" };
  int[] pkColumns;
...
System.out.println("name: " + name + ", " + schema);
  ...
  protected void getData() throws SQLException {
    ...
    synchronized ( cn ) {
      ...
      if ( alpha ) {
        rs = stmt.executeQuery("select distinct SYSTEM_COLUMN_NAME, DATA_TYPE, NUMERIC_SCALE, LENGTH, COLUMN_TEXT, ordinal_position from syscolumns where table_name = '" + this.name + "' and table_schema = '" + schema + "' order by SYSTEM_COLUMN_NAME");
      }
      else {
        rs = stmt.executeQuery("select distinct SYSTEM_COLUMN_NAME, DATA_TYPE, NUMERIC_SCALE, LENGTH, COLUMN_TEXT, ordinal_position from syscolumns where table_name = '" + this.name + "' and table_schema = '" + schema + "' order by ordinal_position");
      }
      while ( rs.next() ) {
      ...
      rs.close();
      list = new ArrayList();
      rs = stmt.executeQuery("select column_position from qsys2/syskeycst a inner join qsys2/syscst b on a.constraint_schema  = b.constraint_schema and a.constraint_name = b.constraint_name where constraint_type = 'PRIMARY KEY' and a.table_name = '" + this.name + "'");
      while ( rs.next() ) {
