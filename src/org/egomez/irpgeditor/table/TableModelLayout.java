		...
		synchronized (cn) {
			...
			if (alpha) {
				rs = stmt.executeQuery(
						"select distinct SYSTEM_COLUMN_NAME, DATA_TYPE, NUMERIC_SCALE, LENGTH, COLUMN_TEXT, ordinal_position from syscolumns where table_name = '"
								+ this.name + "' and table_schema = '" + schema + "' order by SYSTEM_COLUMN_NAME");
			} else {
				rs = stmt.executeQuery(
						"select distinct SYSTEM_COLUMN_NAME, DATA_TYPE, NUMERIC_SCALE, LENGTH, COLUMN_TEXT, ordinal_position from syscolumns where table_name = '"
								+ this.name + "' and table_schema = '" + schema + "' order by ordinal_position");
			}
			while (rs.next()) {
